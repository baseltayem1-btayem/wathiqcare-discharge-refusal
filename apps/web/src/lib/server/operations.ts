...
function normalizeDepartmentInput(input: string | null | undefined): string {
  return (input ?? "").trim().toUpperCase().replace(/[\s-]+/g, "_");
}

export function parseOperationDepartment(
...
export function roleHasCrossDepartmentVisibility(
...
export function departmentForRole(
  role: string | null | undefined,
): OperationDepartment {
  const normalized = (role ?? "").trim().toLowerCase();
...
export async function createOperationNotification(args: {
  ...
  const data: Prisma.OperationNotificationCreateManyInput[] = [
    {
      tenantId: args.tenantId,
...
export async function assignCaseOperation(args: {
  ...
  if (args.toUserId) {
    await createOperationNotification({
      ...
      eventType: currentState.assignedToUserId
        ? "case_reassigned"
        : "case_assigned",
      title: currentState.assignedToUserId
        ? "Case reassigned"
        : "Case assigned",
...
export async function recordCaseStepAction(args: {
  ...
  const completedStepsCount =
    args.action === "step_completed"
      ? Math.min(state.completedStepsCount + 1, state.totalStepsCount)
      : state.completedStepsCount;

  const escalationLevel =
    args.action === "escalation_triggered"
      ? nextEscalationLevel(state.escalationLevel)
...
export async function runSlaSweepForTenant(
  tenantId: string,
  ...
  for (const state of states) {
    ...
    await prisma().caseOperationState.update({
      ...
      data: {
        slaState: nextState,
        waitingTimeMinutes: state.slaDeadline
          ? minutesBetween(state.slaDeadline, now)
          : 0,
      },
    });
