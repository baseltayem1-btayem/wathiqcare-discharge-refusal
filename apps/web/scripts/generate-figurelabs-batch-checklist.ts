/**
 * Generate a human-friendly production checklist for FigureLabs Batch 1.
 *
 * Usage:
 *   npx tsx scripts/generate-figurelabs-batch-checklist.ts
 *
 * Outputs:
 *   docs/clinical-illustrations/batches/figurelabs_batch_01_production_checklist.md
 */

import { writeFileSync, readFileSync } from "node:fs";

const BATCH_CSV = "../../docs/clinical-illustrations/batches/figurelabs_batch_01_priority_20.csv";
const OUTPUT_MD = "../../docs/clinical-illustrations/batches/figurelabs_batch_01_production_checklist.md";
...
function statusFromRow(row: Record<string, string>): string {
  if (row.imageReviewStatus.toLowerCase() === "approved") return "already_approved";
...
function main() {
  ...
  sections.push(`# FigureLabs Batch 1 Production Checklist`);
  sections.push("");
  ...
  for (const row of batchRows) {
    ...
    if (status === "pending_generation") pendingCount++;
  ...
  sections.push("");
  sections.push(`- **Already approved / completed:** ${batchRows.length - pendingCount}`);
  sections.push(`- **Pending FigureLabs generation:** ${pendingCount}`);
  sections.push("");

  writeFileSync(OUTPUT_MD, sections.join("\n"), "utf8");
  console.log(`Wrote Batch 1 production checklist to ${OUTPUT_MD}`);
  console.log(`Pending generation: ${pendingCount} / ${batchRows.length}`);
}
