    ...
    <header className="sticky top-0 z-30 border-b border-[var(--border)] bg-white/95 backdrop-blur">
      <div className="mx-auto w-full max-w-[1600px] px-3 py-2 md:px-5">
        ...
        <div className="mt-2 hidden items-center justify-between gap-3 rounded-xl border border-[var(--border-soft)] bg-slate-50/70 px-3 py-2 md:flex">
          <div className="flex flex-wrap items-center gap-2" aria-label="Quick legal actions">
            {(quickActions || []).map((action) => (
              <TopQuickAction key={action.href} {...action} />
            ))}
          </div>
            ...
            >
              ...
              {(quickActions || []).length > 0 ? (
                <div className="mt-3 border-t border-[var(--border-soft)] pt-3">
                  <div className="mb-2 text-xs font-semibold uppercase tracking-[0.08em] text-slate-500">{quickActionsLabel}</div>
                  <div className="grid grid-cols-1 gap-2">
                    {(quickActions || []).map((action) => (
                      <TopQuickAction key={`mobile-action-${action.href}`} {...action} onNavigate={() => setMobileOpen(false)} />
                    ))}
                  </div>
                </div>
              ) : null}
