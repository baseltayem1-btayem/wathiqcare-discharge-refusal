...
import { getConsentFormById } from "@/lib/server/clinical-knowledge/services";
...
export async function GET(
  ...
  const form = await getConsentFormById(tenantId, id);

  if (!form) {
    return NextResponse.json(
      { ok: true, featureFlagEnabled: true, found: false, error: "Form not found" },
      { status: 404 },
    );
  }
