# Workflow state registry migrated from backend/workflow/workflow_registry.py
...
class WorkflowRegistry:
    def __init__(self, transitions: Mapping[WorkflowState, Iterable[WorkflowState]] = None, initial_state: WorkflowState = WorkflowState.CASE_CREATED):
    ...
    def ensure_transition(self, current: WorkflowState, target: WorkflowState) -> None:
        if not self.can_transition(current, target):
            ...
            raise ValueError(f"Invalid transition from {current} to {target}. Allowed: {allowed}")
