    ...
    <main className="min-h-screen bg-white" dir={isRtl ? "rtl" : "ltr"}>
      ...
      <div className="mx-auto max-w-3xl px-4 py-8 md:px-8 md:py-12">
        ...
        <div className="mb-8 flex items-center gap-4">
          <div className="flex h-14 w-14 flex-shrink-0 items-center justify-center rounded-2xl bg-cyan-100">
            <Shield className="h-7 w-7 text-cyan-700" />
          </div>
          <div>
            <h1 className="text-2xl font-extrabold text-cyan-950">{t("privacyPage.title")}</h1>
            <p className="text-sm text-slate-400">{t("privacyPage.subtitle")}</p>
          </div>
        </div>

        <div className="mb-8 rounded-2xl border border-cyan-100 bg-cyan-50 px-5 py-4 text-sm text-slate-700 leading-relaxed">
          {t("privacyPage.intro")}
        </div>
          ...
          {sections.map((i) => (
            <section key={i}>
              <h2 className="mb-2 text-lg font-bold text-cyan-900">
                {t(`privacyPage.sections.${i}.heading`)}
              </h2>
              <p className="text-slate-600 leading-relaxed">
                {t(`privacyPage.sections.${i}.body`)}
              </p>
            </section>
          ))}
        </div>
