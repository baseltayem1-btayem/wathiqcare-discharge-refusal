...
type TabKey = "overview" | "consents" | "agreements" | "roi" | "archive" | "audit" | "documents";
...
type DocLibEntry = {
  code: string;
  nameEn: string;
  stageKey: DocLibStageKey;
  templateKey?: "discharge_refusal_form" | "financial_responsibility_notice";
  signaturePath?: string;
  onlyIfUninsured?: boolean;
  onlyIfEscalated?: boolean;
};

const DOCUMENT_LIBRARY: DocLibEntry[] = [
  {
    code: "IMC-COM-01",
    nameEn: "Initial Communication",
    stageKey: "initial_communication",
  },
  {
    code: "IMC-SOC-01",
    nameEn: "Social Intervention",
    stageKey: "support_and_intervention",
  },
  {
    code: "IMC-DIS-REF-01",
    nameEn: "Refusal Form",
    stageKey: "refusal_form",
  ...
  {
    code: "IMC-DIS-NOT-01",
    nameEn: "Financial Notice",
    stageKey: "official_notification",
  ...
  },
  {
    ...
    stageKey: "promissory_note",
    onlyIfUninsured: true,
  },
  {
    code: "IMC-ESC-01",
    nameEn: "Escalation",
    stageKey: "escalation",
    onlyIfEscalated: true,
  },
  {
    code: "IMC-TIME-01",
    nameEn: "Timeline",
...
const STAGE_ORDER: Record<string, number> = {
  medical_discharge_decision: 1,
  initial_communication: 2,
  support_and_intervention: 3,
  refusal_form: 4,
  official_notification: 5,
  promissory_note: 6,
  escalation: 7,
  closed: 8,
  closure: 8,
...
function getDocLibStatus(
  entry: DocLibEntry,
  workflow: { current_stage: string; documents: Array<{ template_key: string }>; escalation_required: boolean; insurance_coverage_status: string | null } | null,
  caseDetail: { signed_at?: string | null } | null,
): "required" | "not_generated" | "generated" | "signed" | "missing_signature" {
  if (!workflow) {
  ...
  const docStageOrder =
    entry.stageKey === "promissory_note"
      ? STAGE_ORDER.promissory_note
      : entry.stageKey === "closure"
        ? STAGE_ORDER.closure
        : STAGE_ORDER[entry.stageKey] ?? 0;

  const isStageReached = currentStageOrder >= docStageOrder;
  const isRequired = isStageReached &&
    (!entry.onlyIfUninsured || workflow.insurance_coverage_status === "uninsured") &&
    (!entry.onlyIfEscalated || workflow.escalation_required);

  if (!entry.templateKey) {
    return isRequired ? "required" : "not_generated";
  }

  const generated = workflow.documents.some((d) => d.template_key === entry.templateKey);
  if (!generated) {
    return isRequired ? "required" : "not_generated";
  }

  const signed = Boolean(caseDetail?.signed_at);
  if (signed) {
    return "signed";
  }
...
function getDocLibGeneratedDoc(
  entry: DocLibEntry,
  workflow: { documents: Array<{ template_key: string; view_url: string; download_url: string }> } | null,
): { view_url: string; download_url: string } | null {
  if (!entry.templateKey || !workflow) {
    return null;
  }
  return workflow.documents.find((d) => d.template_key === entry.templateKey) ?? null;
}
    ...
    },
    {
      ...
      element: (
        <button
          ...
          className="inline-flex items-center gap-2 rounded-xl border border-indigo-300 bg-indigo-50 px-3 py-2 text-sm font-semibold text-indigo-800 hover:bg-indigo-100"
        >
          <Library className="h-4 w-4" />
          {t("caseDetails.documents.generateDocumentsBtn")}
        </button>
      ),
    },
    {
            ...
            {activeTab === "documents" ? (
              <section className="rounded-2xl border border-slate-200 p-5">
                <div className="mb-4 flex flex-wrap items-start justify-between gap-2">
                  <div>
                    <h2 className="text-base font-semibold text-slate-900">{t("caseDetails.documents.title")}</h2>
                    <p className="mt-1 text-sm text-slate-600">{t("caseDetails.documents.subtitle")}</p>
                  </div>
                  <button
                    type="button"
                    onClick={() => void loadCaseData()}
                ...
                {!workflow ? (
                  <div className="rounded-lg border border-amber-200 bg-amber-50 p-4 text-sm text-amber-800">
                    {t("caseDetails.documents.noWorkflow")}
                  </div>
                ) : (
                      ...
                      <thead className="bg-slate-50">
                        <tr>
                          <th className="px-3 py-2 text-left font-semibold text-slate-700">
                            {t("caseDetails.archive.document")}
                          </th>
                          <th className="px-3 py-2 text-left font-semibold text-slate-700">
                            {t("caseDetails.documents.stage")}
                          </th>
                          <th className="px-3 py-2 text-left font-semibold text-slate-700">
                            {t("caseDetails.documents.status")}
                          </th>
                          <th className="px-3 py-2 text-left font-semibold text-slate-700">
                            {t("caseDetails.documents.actions")}
                          </th>
                        </tr>
                      </thead>
                      <tbody>
                        {DOCUMENT_LIBRARY.map((entry) => {
                          const docStatus = getDocLibStatus(entry, workflow, caseDetail);
                          const generatedDoc = getDocLibGeneratedDoc(entry, workflow);
                              ...
                              case "required":
                                return (
                                  <span className="rounded-full bg-amber-100 px-2 py-1 text-xs font-semibold text-amber-800">
                                    {t("caseDetails.documents.statusRequired")}
                                  </span>
                                );
                              case "not_generated":
                                return (
                                  <span className="rounded-full bg-slate-100 px-2 py-1 text-xs font-medium text-slate-600">
                                    {t("caseDetails.documents.statusNotGenerated")}
                                  </span>
                                );
                              case "generated":
                                return (
                                  <span className="rounded-full bg-emerald-100 px-2 py-1 text-xs font-semibold text-emerald-700">
                                    {t("caseDetails.documents.statusGenerated")}
                                  </span>
                                );
                              case "signed":
                                return (
                                  <span className="rounded-full bg-sky-100 px-2 py-1 text-xs font-semibold text-sky-700">
                                    {t("caseDetails.documents.statusSigned")}
                                  </span>
                                );
                              case "missing_signature":
                                return (
                                  <span className="rounded-full bg-rose-100 px-2 py-1 text-xs font-semibold text-rose-700">
                                    {t("caseDetails.documents.statusMissingSignature")}
                                  </span>
                                );
                            }
                          })();
                          ...
                          return (
                            <tr key={entry.code} className="border-t border-slate-100">
                              <td className="px-3 py-3">
                                <p className="font-medium text-slate-900">{entry.code}</p>
                                <p className="text-xs text-slate-500">{entry.nameEn}</p>
                              </td>
                              <td className="px-3 py-3 text-xs text-slate-600">
                                {t(`caseDetails.documents.stageLabel.${entry.stageKey}`)}
                              </td>
                              <td className="px-3 py-3">{statusBadge}</td>
                              <td className="px-3 py-3">
                                <div className="flex flex-wrap gap-1.5">
                                  {!generatedDoc && entry.templateKey ? (
                                    <button
                                      type="button"
                                      onClick={() => {
                                        if (entry.templateKey) {
                                          handleGenerateDocument(
                                            entry.templateKey === "discharge_refusal_form"
                                              ? "discharge_refusal_form"
                                              : "financial_responsibility_notice"
                                          );
                                        }
                                    ...
                                    >
                                      ...
                                      {t("caseDetails.documents.generate")}
                                    </button>
                                  ) : null}

                                  {generatedDoc ? (
                                    <a
                                      href={generatedDoc.view_url}
                                      target="_blank"
                                  ...
                                  {generatedDoc ? (
                                    ...
                                    >
                                      ...
                                      {t("caseDetails.documents.download")}
                                    </a>
                                  ) : null}

                                  {generatedDoc && docStatus === "missing_signature" && entry.signaturePath ? (
                                    <button
                                      type="button"
                                      onClick={() => {
                                        const path = entry.signaturePath;
                                        if (path) {
                                          handleSendForSignature(path);
                                        }
                                      }}
                                      className="inline-flex items-center gap-1 rounded-lg border border-sky-300 bg-sky-50 px-2 py-1 text-xs font-medium text-sky-800 hover:bg-sky-100"
                                    >
                                      <Send className="h-3 w-3" />
                                      {t("caseDetails.documents.sendForSignature")}
                                    </button>
                                  ) : null}
                                </div>
                              </td>
