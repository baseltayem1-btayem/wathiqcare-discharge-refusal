...
export default function LoginPage() {
  ...
  const copy = {
    ...
    imcSoon: isRtl ? "سيتوفر قريباً" : "Available soon",
          ...
          <div className="relative z-10 flex h-full w-full max-w-[720px] flex-col">
            ...
            <div className="mt-10 space-y-4">
              {[
                {
                  icon: ShieldCheck,
                  title: isRtl ? "خصوصية طبية" : "Medical Privacy",
                  desc: isRtl ? "حماية عالية للبيانات الحساسة." : "High protection for sensitive data.",
                },
                {
                  icon: CheckCircle2,
                  title: isRtl ? "امتثال مستمر" : "Continuous Compliance",
                  desc: isRtl ? "متابعة متطلبات التنظيم بشكل آني." : "Real-time alignment with regulations.",
                },
                {
                  icon: Scale,
                  title: isRtl ? "حوكمة واضحة" : "Clear Governance",
                  desc: isRtl ? "قرارات دقيقة مدعومة بسياق قانوني." : "Precise decisions with legal context.",
                },
              ].map((item) => (
        ...
        <section className="auth-form-wrap flex items-center justify-center px-5 py-8 sm:px-10 lg:px-14 lg:py-12">
          <div className="w-full max-w-[600px]">
            <div className="mb-6 flex items-center justify-between gap-3">
              <Link href={isRtl ? "/ar" : "/en"} className="home-link">
            ...
            <div className="auth-card rounded-[32px] border border-[#d9e3f0] bg-white p-6 sm:p-10">
              <div className="mb-7 space-y-2">
                <h2 className="text-[2rem] font-semibold leading-tight text-[#18324f]">{copy.title}</h2>
                <p className="text-[1.01rem] text-[#5e7289]">{copy.subtitle}</p>
              </div>

              <form onSubmit={handlePasswordSubmit} className="space-y-5" noValidate>
                <div className="space-y-2.5">
                  <label htmlFor="email" className="text-sm font-semibold text-[#324862]">
                    {copy.emailLabel}
                  </label>
                ...
                <div className="space-y-2.5">
                  <label htmlFor="password" className="text-sm font-semibold text-[#324862]">
                    {copy.passwordLabel}
                  </label>
                  <div className="auth-input-wrap">
                ...
                <div className="flex items-center justify-between gap-3 text-sm">
                  ...
                  <Link href="/auth/password-reset" className="font-semibold text-[#1e598f] hover:text-[#113d6e]">
                    {copy.forgotPassword}
                  </Link>
                </div>

                {error ? (
                  <div role="alert" className="rounded-2xl border border-[#f2c7c7] bg-[#fff5f5] px-4 py-3 text-sm text-[#8f3232]">
                    {error}
                  </div>
                ) : null}
                ...
                <button type="button" disabled className="auth-secondary-btn" aria-disabled="true">
                  <span className="font-semibold">{copy.imcBtn}</span>
                  <span className="text-xs text-[#6f8197]">{copy.imcSoon}</span>
                </button>
              </form>

              <footer className="mt-7 flex flex-wrap items-center gap-x-6 gap-y-3 border-t border-[#e9eff6] pt-5 text-sm text-[#5b6f86]">
                <a href="mailto:support@wathiqcare.online" className="inline-flex items-center gap-2 hover:text-[#113d6e]">
                  <Phone className="h-4 w-4" />
                  {copy.support}
                </a>
                <Link href={isRtl ? "/ar/privacy" : "/en/privacy"} className="inline-flex items-center gap-2 hover:text-[#113d6e]">
                  <ShieldCheck className="h-4 w-4" />
                  {copy.privacy}
                </Link>
