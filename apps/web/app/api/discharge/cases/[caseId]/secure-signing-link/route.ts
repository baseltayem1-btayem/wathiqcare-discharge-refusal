...
function getWorkflow(metadata: Prisma.JsonValue | null | undefined): SecureSigningWorkflow | null {
  ...
  const discharge = secureSigning.discharge;
  if (!discharge || typeof discharge !== "object" || Array.isArray(discharge)) return null;
  return discharge as SecureSigningWorkflow;
}
...
export async function POST(request: NextRequest, { params }: RouteContext) {
  try {
    ...
    const patientMobile = body?.mobileNumber?.trim() || getPatientMobile(metadata);

    const workflow = await sendModuleSecureSigningLink({
      tenantId,
      initiatedBy: auth.sub,
      moduleKey: "discharge_refusal",
