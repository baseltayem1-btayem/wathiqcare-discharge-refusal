/**
 * Prepare a clean FigureLabs handoff package for Batch 5.
 *
 * Usage:
 *   npx tsx scripts/prepare-figurelabs-batch-03-handoff.ts
 *
 * Outputs:
 *   figurelabs_batch_05_handoff_package.zip
 */
...
const BATCH_CSV = "../../docs/clinical-illustrations/batches/figurelabs_batch_05_priority_20.csv";
const BATCH_CHECKLIST = "../../docs/clinical-illustrations/batches/figurelabs_batch_05_production_checklist.md";
const OUTPUT_DIR = "../../.tmp-handoff/figurelabs_batch_05_handoff_package";
...
function main() {
  ...
  writeFileSync(`${OUTPUT_DIR}/figurelabs_batch_05_priority_20.csv`, toCsv(rowsWithStatus, statusColumns), "utf8");

  // 2. Copy checklist.
  copyFileSync(BATCH_CHECKLIST, `${OUTPUT_DIR}/figurelabs_batch_05_production_checklist.md`);

  // 3. Prompts file — one copy/paste prompt per procedure.
  const promptsLines: string[] = ["# FigureLabs Batch 5 Prompts", ""];
  ...
  writeFileSync(`${OUTPUT_DIR}/figurelabs_batch_05_prompts.md`, promptsLines.join("\n"), "utf8");
  ...
  writeFileSync(
    `${OUTPUT_DIR}/figurelabs_batch_05_expected_filenames.csv`,
  ...
  writeFileSync(
    `${OUTPUT_DIR}/figurelabs_batch_05_expected_public_paths.csv`,
  ...
  reviewRows.push({
    sequence: "-",
    procedureNameEn: "Batch 5 duplicate check",
...
## Output naming
Use the exact filenames in \`figurelabs_batch_05_expected_filenames.csv\`.
Do not rename files.

## Output paths
Store images under the project paths in \`figurelabs_batch_05_expected_public_paths.csv\`.

## Authorization certificates
Provide a separate authorization certificate for each illustration.
Expected certificate filenames are listed in \`figurelabs_batch_05_expected_filenames.csv\`.

## Prompts
Copy/paste prompts are in \`figurelabs_batch_05_prompts.md\`.

## Manual review items
Arabic translations marked "subject to clinical review" are in
\`figurelabs_batch_05_manual_review_items.csv\`.
...
`;
  ...
  const engineeringReadme = `# Engineering Integration Guide — Batch 5

## After FigureLabs delivers the PNGs
1. Receive the generated PNG files and authorization certificates.
2. Copy the PNGs into \`apps/web/public/educational/clinical-illustrations/\` using the exact folder structure and filenames from \`figurelabs_batch_05_expected_public_paths.csv\`.
...
4. Open \`apps/web/src/lib/server/clinical-knowledge/migration/figurelabs-batch-data.ts\`.
   - Convert the Batch 5 keys into full \`BatchIllustration\` records.
   ...
   - Apply the clinically reviewed Arabic names from \`figurelabs_batch_05_manual_review_items.csv\`.
5. Update \`apps/web/src/lib/server/clinical-knowledge/migration/seed-from-imc.ts\` to iterate the Batch 5 approved records (same pattern as Batch 1).
...
- imageReviewStatus: pending_generation
- integration_status: not_integrated
- Seed mapping: not yet wired to Batch 5 images.
`;
  ...
  const zipName = "figurelabs_batch_05_handoff_package.zip";
  ...
  // Prefer system \`zip\`.
  try {
    execSync(`zip -r "${zipName}" "figurelabs_batch_05_handoff_package"`, {
      cwd: outputDirParent,
      stdio: "ignore",
    });
    zipped = true;
  ...
  if (!zipped) {
    try {
      execSync(
        `powershell -Command "Compress-Archive -Path '${outputDirParent.replace(/\//g, "\\\\")}\\\\figurelabs_batch_05_handoff_package' -DestinationPath '${zipTarget.replace(/\//g, "\\\\")}' -Force"`,
        { stdio: "ignore" },
      );
      zipped = true;
    } catch {
