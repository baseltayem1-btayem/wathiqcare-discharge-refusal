...
import { MOCK_PATIENTS, MOCK_ENCOUNTERS } from "../lib/mock-data";
...
export function PatientEncounterSelector({
  ...
  encounters: externalEncounters,
  patientsLoading: externalPatientsLoading,
  encountersLoading: externalEncountersLoading,
  error: externalError,
  onSearchQueryChange,
}: PatientEncounterSelectorProps) {
  const isProduction = externalPatients !== undefined;
  const [query, setQuery] = useState("");
  const [expanded, setExpanded] = useState(!selectedPatient);

  const mockFilteredPatients = useMemo(() => {
  ...
  const filteredPatients = isProduction
    ? externalPatients ?? []
    : mockFilteredPatients;

  const patientEncounters = selectedPatient
    ? (isProduction ? externalEncounters : MOCK_ENCOUNTERS[selectedPatient.id]) ?? []
    : [];

  const patientsLoading = isProduction ? externalPatientsLoading : false;
  const encountersLoading = isProduction ? externalEncountersLoading : false;
  const error = isProduction ? externalError : undefined;
    ...
    <Card variant="default" className="overflow-hidden" id="section-patient">
      ...
      {expanded && (
        <CardContent className="p-5 space-y-5">
          ...
          {!patientsLoading && query.length >= 2 && filteredPatients.length === 0 && (
