// Phase 40E — targeted E2E regression capture (read-only, no mutations)
// Re-verifies physician Consent Builder default landing AND patient/sign route preservation.
...
const OUT_DIR = path.join(__dirname, 'docs', 'production-readiness', 'phase40e-targeted-e2e-regression-screenshots');
fs.mkdirSync(OUT_DIR, { recursive: true });

// Candidate controlled tokens from prior sessions; try in order. Do NOT create new.
...
(async () => {
  ...
  // === PATIENT JOURNEY PRESERVATION (no new tokens; just observe existing route mounts) ===
  // Use a fresh anon context so no physician cookie leaks into patient flow.
  ...
  for (const t of PATIENT_TOKEN_CANDIDATES) {
    ...
    try {
      ...
      shots.push({ name: `08_patient_workflow_${t.slice(0, 8)}`, url, status, file: path.relative(__dirname, file).replace(/\\/g, '/'), bytes });
  ...
  const meta = {
    ...
    physicianShots: shots.filter(s => !s.name.startsWith('08_')),
    patientJourneyAttempts: tokenResults,
    apiSmoke: { 'GET /api/health/runtime': healthJson },
  };
  fs.writeFileSync(path.join(OUT_DIR, 'phase40e-screenshots-metadata.json'), JSON.stringify(meta, null, 2));
