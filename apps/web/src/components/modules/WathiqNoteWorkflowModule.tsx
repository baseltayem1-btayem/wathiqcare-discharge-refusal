...
function NoteBuilder({
  ...
  async function issueNotePrototype() {
    if (!caseId.trim()) {
      showToast({ type: "warning", message: txt(lang, "معرّف الحالة مطلوب لأن واجهة البرمجة الحالية تربط السند بملف حالة قائم.", "Case ID is required because the current API links the note to an existing case.") });
      return;
    }
    ...
    try {
      ...
      if (smsOk) {
        ...
        showToast({
          ...
          message: txt(
            lang,
            `تم إنشاء السند، لكن فشل إرسال رسالة الجوال إلى ${mobile}. تحقق من إعدادات بوابة الرسائل أو أعد الإرسال من بطاقة النتيجة.`,
        ...
        <div className="space-y-5">
          ...
          {activeStep === "note_details" ? (
            <ShellCard className="p-5">
              ...
              <div className="grid grid-cols-2 gap-4">
                <Field label="Case ID" value={caseId} onChange={setCaseId} placeholder={txt(lang, "أدخل معرّف الحالة من النظام", "Enter existing system Case ID")} />
  ...
  return (
    <div className="space-y-5">
      <PageTitle
        ...
        subtitle={txt(
          lang,
          "إدارة حسابات مستخدمي وثيق نوت وربط كل مستخدم بدور تشغيلي واضح داخل دورة حياة السند.",
  ...
  return (
    <div className="space-y-5">
      ...
      <ShellCard>
        ...
        {error ? (
          <div className="m-5 rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm leading-6 text-amber-800">
            {txt(lang, "تعذر تحميل مصفوفة قاعدة البيانات. سيتم عرض مصفوفة وثيق نوت التشغيلية المقترحة أدناه.", "Unable to load the database matrix. The proposed WathiqNote operating matrix is shown below.")}
            <div className="mt-2 font-mono text-xs">{error}</div>
          </div>
        ) : null}
      ...
      <ShellCard>
        <div className="border-b border-slate-200 px-5 py-4">
          <h2 className="text-lg font-bold text-[#073763]">{txt(lang, "مصفوفة وثيق نوت التشغيلية", "WathiqNote Operating Matrix")}</h2>
          <p className="mt-1 text-xs text-slate-500">
      ...
      <ShellCard className="p-5">
        ...
        <div className="mt-4 grid grid-cols-3 gap-4">
          <div className="rounded-xl border border-blue-100 bg-blue-50 p-4">
            ...
            <p className="mt-2 text-sm leading-6 text-blue-800">{txt(lang, "يقتصر على جهة الإصدار أو المسؤول بعد اكتمال البيانات والمراجعة.", "Restricted to Issuer or Admin after data completion and review.")}</p>
          </div>
          <div className="rounded-xl border border-emerald-100 bg-emerald-50 p-4">
            <div className="font-bold text-emerald-900">{txt(lang, "الوفاء", "Settlement")}</div>
            <p className="mt-2 text-sm leading-6 text-emerald-800">{txt(lang, "يقتصر على المالية أو المسؤول مع مرجع مالي وتاريخ تسوية.", "Restricted to Finance or Admin with payment reference and settlement date.")}</p>
          </div>
    ...
    <div className="space-y-5">
      ...
      <ShellCard className="p-5">
        ...
        <div className="mt-4 grid gap-3">
          {[
            ["إصدار سندات حتى ١٠٠٠٠ ريال سعودي", "جهة الإصدار بعد المراجعة", "Issue notes up to SAR 10,000", "Issuer after review"],
            ["إصدار السند فوق 10,000 ريال", "Issuer + Finance Reviewer", "Issue notes above SAR 10,000", "Issuer + Finance Reviewer"],
            ["إبطال سند غير موقع", "القانوني أو المسؤول مع سبب", "Void unsigned note", "Legal or Admin with reason"],
...
function AuditTrailScreen({ lang }: { lang: Lang }) {
  ...
  return (
    <div className="space-y-5">
      ...
      <ShellCard>
        <div className="border-b border-slate-200 px-5 py-4">
          <h2 className="text-lg font-bold text-[#073763]">{txt(lang, "أحداث وثيق نوت", "WathiqNote Events")}</h2>
        </div>
        <div className="divide-y divide-slate-100">
