...
export async function GET(request: NextRequest) {
  ...
  if (!pdf?.publicPath) {
    return NextResponse.json(
      {
        success: false,
        error: "Official approved consent PDF is not uploaded",
        detail:
          "This item exists in the approved consent library, but the official approved PDF file has not been uploaded or mapped. A generated placeholder will not be shown as an approved consent.",
        id,
        title,
        expectedFileName,
        requiredPath: `/approved-consents/${expectedFileName}`,
      },
      { status: 404 },
    );
  }
