/**
 * Prepare a clean FigureLabs handoff package for Batch 2.
 *
 * Usage:
 *   npx tsx scripts/prepare-figurelabs-batch-02-handoff.ts
 *
 * Outputs:
 *   figurelabs_batch_02_handoff_package.zip
 */
...
const MANUAL_REVIEW_ITEMS = [
  {
    sequence: 9,
    procedureNameEn: "Angiogram with mechanical Thrombectomy (any body part)",
    field: "procedureNameAr",
    proposedValueAr: "تصوير الأوعية مع استئصال الخثرة ميكانيكيًا، لأي موضع بالجسم",
    status: "subject to clinical review",
  },
  {
    sequence: 14,
    procedureNameEn: "Aspiration Drainage Under Imaging",
    field: "procedureNameAr",
    proposedValueAr: "بزل أو تصريف السوائل تحت توجيه التصوير",
    status: "subject to clinical review",
  },
  {
    sequence: 18,
    procedureNameEn: "Blood and blood products transfusion consent",
    field: "procedureNameAr",
    proposedValueAr: "موافقة نقل الدم ومشتقاته",
    status: "subject to clinical review",
  },
];
...
function main() {
  ...
  writeFileSync(`${OUTPUT_DIR}/figurelabs_batch_02_priority_20.csv`, toCsv(rowsWithStatus, statusColumns), "utf8");

  // 2. Copy checklist.
  copyFileSync(BATCH_CHECKLIST, `${OUTPUT_DIR}/figurelabs_batch_02_production_checklist.md`);

  // 3. Prompts file — one copy/paste prompt per procedure.
  const promptsLines: string[] = ["# FigureLabs Batch 2 Prompts", ""];
  ...
  writeFileSync(`${OUTPUT_DIR}/figurelabs_batch_02_prompts.md`, promptsLines.join("\n"), "utf8");
  ...
  writeFileSync(
    `${OUTPUT_DIR}/figurelabs_batch_02_expected_filenames.csv`,
  ...
  writeFileSync(
    `${OUTPUT_DIR}/figurelabs_batch_02_expected_public_paths.csv`,
  ...
  const reviewRows = MANUAL_REVIEW_ITEMS.map((item) => ({
  ...
  reviewRows.push({
    sequence: "-",
    procedureNameEn: "Batch 2 duplicate check",
...
## Output naming
Use the exact filenames in \`figurelabs_batch_02_expected_filenames.csv\`.
Do not rename files.

## Output paths
Store images under the project paths in \`figurelabs_batch_02_expected_public_paths.csv\`.

## Authorization certificates
Provide a separate authorization certificate for each illustration.
Expected certificate filenames are listed in \`figurelabs_batch_02_expected_filenames.csv\`.

## Prompts
Copy/paste prompts are in \`figurelabs_batch_02_prompts.md\`.

## Manual review items
Arabic translations marked "subject to clinical review" are in
\`figurelabs_batch_02_manual_review_items.csv\`.
...
`;
  ...
  const engineeringReadme = `# Engineering Integration Guide — Batch 2

## After FigureLabs delivers the PNGs
1. Receive the generated PNG files and authorization certificates.
2. Copy the PNGs into \`apps/web/public/educational/clinical-illustrations/\` using the exact folder structure and filenames from \`figurelabs_batch_02_expected_public_paths.csv\`.
...
4. Open \`apps/web/src/lib/server/clinical-knowledge/migration/figurelabs-batch-data.ts\`.
   - Convert the Batch 2 keys into full \`BatchIllustration\` records.
   - Set \`imageReviewStatus: "approved"\` and \`patientFacing: true\`.
   - Set the actual \`procedureImageUrl\` paths.
   - Apply the clinically reviewed Arabic names from \`figurelabs_batch_02_manual_review_items.csv\`.
5. Update \`apps/web/src/lib/server/clinical-knowledge/migration/seed-from-imc.ts\` to iterate the Batch 2 approved records (same pattern as Batch 1).
6. Regenerate the registry:
   \`\`\`bash
   cd apps/web
   npx tsx scripts/generate-figurelabs-registry.ts
   npx tsx scripts/generate-figurelabs-batch.ts
   npx tsx scripts/generate-figurelabs-batch-02.ts
   npx tsx scripts/generate-figurelabs-batch-checklist.ts
   npx tsx scripts/generate-figurelabs-batch-02-checklist.ts
   \`\`\`
...
- imageReviewStatus: pending_generation
- integration_status: not_integrated
- Seed mapping: not yet wired to Batch 2 images.
`;
  ...
  // Prefer system `zip`.
  try {
    execSync(`zip -r "${zipName}" "figurelabs_batch_02_handoff_package"`, {
      cwd: outputDirParent,
      stdio: "ignore",
    });
    zipped = true;
  ...
  if (!zipped) {
    try {
      execSync(
        `powershell -Command "Compress-Archive -Path '${outputDirParent.replace(/\//g, "\\\\")}\\\\figurelabs_batch_02_handoff_package' -DestinationPath '${zipTarget.replace(/\//g, "\\\\")}' -Force"`,
        { stdio: "ignore" },
      );
      zipped = true;
    } catch {
  ...
  console.log(`Manual review items: ${MANUAL_REVIEW_ITEMS.length}`);
