...
export function StepSend({
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/modules/informed-consents/documents/${encodeURIComponent(linkedDocumentId)}/secure-signing`, {
        method: "POST",
        headers: {
      ...
      if (!response.ok) {
        throw new Error(result?.error || result?.message || `Secure signing request failed with status ${response.status}`);
      }
      ...
      setSendError(error instanceof Error ? error.message : (lang === "en" ? "Failed to send secure signing link." : ar.sendFailed));
    } finally {
      setIsSending(false);
    }
  };
  ...
  if (sent && workflow) {
    return (
      <div className="p-8 flex flex-col items-center justify-center min-h-96 text-center space-y-4" dir={dir}>
        <div className="w-full max-w-xl bg-emerald-50 border border-emerald-200 rounded px-4 py-3 text-left" data-testid="step-send-success-real-banner">
        ...
        <div>
          <h2 className="text-emerald-700">{lang === "en" ? "Secure Consent Link Created" : ar.linkSent}</h2>
        ...
        <div className="bg-white border border-[#D8DCE3] rounded-lg p-5 w-full max-w-xl text-left">
          <div className="space-y-2 text-sm">
            {[
              { label: lang === "en" ? "Sent to" : ar.sentTo, value: workflow.recipientMobile || displayMobile },
              { label: lang === "en" ? "Email" : ar.emailAddress, value: workflow.recipientEmail || displayEmail },
              { label: lang === "en" ? "SMS status" : "???? ??????? ??????", value: workflow.smsDeliveryStatus || "n/a" },
              { label: lang === "en" ? "Email status" : "???? ?????? ??????????", value: workflow.emailDeliveryStatus || "n/a" },
              { label: lang === "en" ? "Language" : ar.language, value: patientLang === "both" ? "Arabic & English" : patientLang.toUpperCase() },
              { label: lang === "en" ? "Expires" : ar.expires, value: `${expiryHours} hours` },
              { label: lang === "en" ? "Session" : ar.reference, value: workflow.sessionId || linkedDocumentId || "n/a" },
            ].map(item => (
              <div key={item.label} className="flex justify-between gap-4">
                <span className="text-[#6B7280]">{item.label}</span>
                <span className="font-medium text-[#2F2F2F] font-mono text-right break-all">{item.value}</span>
              </div>
            ))}

            <div className="pt-3 mt-3 border-t border-[#EEF1F5]">
              <div className="text-xs text-[#6B7280] mb-1">{lang === "en" ? "Secure signing URL" : ar.signingUrl}</div>
              <div className="flex items-center gap-2">
                ...
                >
                  ...
                  {copied ? (lang === "en" ? "Copied" : ar.copied) : (lang === "en" ? "Copy" : ar.copyLink)}
                </button>
                <a
        ...
        <ClinicalBadge variant="info" label={lang === "en" ? "Tracking consent in Status tab" : ar.tracking} />
      </div>
    );
  }
  ...
  return (
    <div className="p-8 space-y-6" dir={dir}>
      <div className="flex items-start gap-2 bg-blue-50 border border-blue-200 rounded px-4 py-3" data-testid="step-send-real-delivery-banner">
      ...
      <div>
        <h2 className="text-[#002B5C]">{lang === "en" ? "Send Secure Consent Link" : ar.sendSecureConsentLink}</h2>
        <p className="text-sm text-[#6B7280] mt-1">
          {lang === "en" ? "Confirm patient contact details and OTP method before sending." : ar.subtitle}
        </p>
      </div>

      {(documentError || sendError || !linkedDocumentId || !documentReady) && (
      ...
      <div className="bg-white border border-[#D8DCE3] rounded-lg p-5">
        <h3 className="text-[#2F2F2F] mb-4">{lang === "en" ? "Patient Contact Details" : ar.patientContactDetails}</h3>
      ...
      <div className="bg-white border border-[#D8DCE3] rounded-lg p-5">
        <h3 className="text-[#2F2F2F] mb-4">{lang === "en" ? "OTP Signing Method" : ar.otpSigningMethod}</h3>
        <div className="grid grid-cols-3 gap-3">
          {[
            { id: "sms" as const, label: lang === "en" ? "SMS Only" : ar.smsOnly, icon: Phone, desc: lang === "en" ? "OTP sent to mobile" : ar.otpMobile },
            { id: "email" as const, label: lang === "en" ? "Email Only" : ar.emailOnly, icon: Mail, desc: lang === "en" ? "OTP sent to email" : ar.otpEmail },
            { id: "both" as const, label: lang === "en" ? "SMS + Email" : ar.smsEmail, icon: Shield, desc: lang === "en" ? "Dual verification" : ar.dualVerification },
          ].map(method => (
            <div
              key={method.id}
      ...
      <div className="grid grid-cols-2 gap-4">
          ...
          <div className="space-y-2">
            {[
              ...
              <label key={item.id} className="flex items-center gap-2 cursor-pointer">
                <input type="radio" checked={patientLang === item.id} onChange={() => setPatientLang(item.id)} className="accent-[#002B5C]" />
        ...
        <div className="bg-white border border-[#D8DCE3] rounded-lg p-5">
          ...
          <div className="space-y-3">
            <div>
              <label className="text-xs font-medium text-[#6B7280] block mb-1">{lang === "en" ? "Expiry Duration" : ar.expiryDuration}</label>
            ...
            <div>
              <label className="text-xs font-medium text-[#6B7280] block mb-1">{lang === "en" ? "Max Resend Attempts" : ar.maxResend}</label>
      ...
      <div className={`bg-[#002B5C]/5 border border-[#002B5C]/20 rounded-lg p-5 ${physicianConfirmed ? "border-emerald-300 bg-emerald-50" : ""}`}>
        <div className="flex items-center gap-2 mb-3">
          <Shield className="w-4 h-4 text-[#002B5C]" />
          <span className="text-sm font-semibold text-[#002B5C]">{lang === "en" ? "Physician Final Confirmation" : ar.finalConfirmation}</span>
        </div>
        ...
        <label className="flex items-center gap-3 cursor-pointer">
          ...
          <span className="text-sm font-semibold text-[#002B5C]">
            {lang === "en" ? "I confirm all the above statements are true and authorize sending." : ar.checkbox}
          </span>
        </label>
      </div>
      ...
      {!physicianConfirmed && (
        <div className="flex items-center gap-2 bg-amber-50 border border-amber-200 rounded px-4 py-2.5">
          ...
          <span className="text-xs text-amber-800">{lang === "en" ? "Send button is disabled until physician confirmation is checked." : ar.disabled}</span>
        </div>
      )}
      ...
      <div className="flex justify-between">
        ...
        >
          ...
          {isSending ? (lang === "en" ? "Sending..." : ar.sending) : (lang === "en" ? "Send Consent Link" : ar.send)}
        </button>
      </div>
    </div>
  );
