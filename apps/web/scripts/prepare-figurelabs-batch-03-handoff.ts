/**
 * Prepare a clean FigureLabs handoff package for Batch 3.
 *
 * Usage:
 *   npx tsx scripts/prepare-figurelabs-batch-03-handoff.ts
 *
 * Outputs:
 *   figurelabs_batch_03_handoff_package.zip
 */
...
const BATCH_CSV = "../../docs/clinical-illustrations/batches/figurelabs_batch_03_priority_20.csv";
const BATCH_CHECKLIST = "../../docs/clinical-illustrations/batches/figurelabs_batch_03_production_checklist.md";
const OUTPUT_DIR = "../../.tmp-handoff/figurelabs_batch_03_handoff_package";
...
const MANUAL_REVIEW_ITEMS = [
  {
    sequence: 5,
    procedureNameEn: "Burns (Child-Young Person)",
    field: "procedureNameAr",
    proposedValueAr: "Burns (Child-Young Person)",
    status: "subject to clinical review",
  },
  {
    sequence: 10,
    procedureNameEn: "Catheter Check with IV Iodinated Contrast",
    field: "procedureNameAr",
    proposedValueAr: "Catheter Check with IV Iodinated Contrast",
    status: "subject to clinical review",
  },
  {
    sequence: 18,
    procedureNameEn: "Cholecystectomy Open and Exploration of the Common Bile Duct",
    field: "procedureNameAr",
    proposedValueAr: "Cholecystectomy Open and Exploration of the Common Bile Duct",
    status: "subject to clinical review",
  },
  {
    sequence: 19,
    procedureNameEn: "Cholecystoduodenostomy - Enterostomy",
    field: "procedureNameAr",
    proposedValueAr: "Cholecystoduodenostomy - Enterostomy",
    status: "subject to clinical review",
  },
];
...
function main() {
  ...
  writeFileSync(`${OUTPUT_DIR}/figurelabs_batch_03_priority_20.csv`, toCsv(rowsWithStatus, statusColumns), "utf8");

  // 2. Copy checklist.
  copyFileSync(BATCH_CHECKLIST, `${OUTPUT_DIR}/figurelabs_batch_03_production_checklist.md`);

  // 3. Prompts file — one copy/paste prompt per procedure.
  const promptsLines: string[] = ["# FigureLabs Batch 3 Prompts", ""];
  ...
  writeFileSync(`${OUTPUT_DIR}/figurelabs_batch_03_prompts.md`, promptsLines.join("\n"), "utf8");
  ...
  writeFileSync(
    `${OUTPUT_DIR}/figurelabs_batch_03_expected_filenames.csv`,
  ...
  writeFileSync(
    `${OUTPUT_DIR}/figurelabs_batch_03_expected_public_paths.csv`,
  ...
  reviewRows.push({
    sequence: "-",
    procedureNameEn: "Batch 3 duplicate check",
  ...
  writeFileSync(
    `${OUTPUT_DIR}/figurelabs_batch_03_manual_review_items.csv`,
    toCsv(reviewRows, ["sequence", "procedureNameEn", "field", "proposedValueAr", "status"]),
    "utf8",
  );

  // 7. README for FigureLabs.
  const figurelabsReadme = `# FigureLabs Batch 3 Handoff Package
...
`;
  ...
  const engineeringReadme = `# Engineering Integration Guide — Batch 3
...
`;
  ...
  const zipName = "figurelabs_batch_03_handoff_package.zip";
  ...
  // Prefer system \`zip\`.
  try {
    execSync(`zip -r "${zipName}" "figurelabs_batch_03_handoff_package"`, {
      cwd: outputDirParent,
      stdio: "ignore",
    });
    zipped = true;
  ...
  if (!zipped) {
    try {
      execSync(
        `powershell -Command "Compress-Archive -Path '${outputDirParent.replace(/\//g, "\\\\")}\\\\figurelabs_batch_03_handoff_package' -DestinationPath '${zipTarget.replace(/\//g, "\\\\")}' -Force"`,
        { stdio: "ignore" },
      );
      zipped = true;
    } catch {
