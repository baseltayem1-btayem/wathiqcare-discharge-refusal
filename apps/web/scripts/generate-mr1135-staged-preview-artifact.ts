/**
 * Generate MR1135 staged Filled Preview acceptance artifact.
 *
 * Uses the same runtime pathway as the production draft-pdf endpoint
 * (renderAcroFormFilledDraftPreview) to produce:
 *   - test-output/mr1135-staged-preview/MR1135-Staged-Filled-Preview.pdf
 *   - test-output/mr1135-staged-preview/MR1135-Staged-Filled-Preview-page-{1..5}.png
 *   - test-output/mr1135-staged-preview/staged-preview-binding-report.json
 *   - test-output/mr1135-staged-preview/staged-preview-visual-qa-report.json
 */
...
const OUTPUT_DIR = path.join(process.cwd(), "test-output", "mr1135-staged-preview");
...
function loadCanonicalPdf(): Uint8Array {
  ...
  throw new Error("Canonical amputation PDF not found for artifact generation");
}
...
async function main(): Promise<void> {
  ...
  try {
    ...
    const pdfPath = path.join(OUTPUT_DIR, "MR1135-Staged-Filled-Preview.pdf");
    ...
    for (let pageIndex = 0; pageIndex < pageCount; pageIndex++) {
      ...
      const pngPath = path.join(OUTPUT_DIR, `MR1135-Staged-Filled-Preview-page-${pageIndex + 1}.png`);
    ...
    fs.writeFileSync(
      path.join(OUTPUT_DIR, "staged-preview-binding-report.json"),
    ...
    fs.writeFileSync(
      path.join(OUTPUT_DIR, "staged-preview-visual-qa-report.json"),
      JSON.stringify(visualQaReport, null, 2),
    );

    console.log(`Wrote ${pdfPath}`);
    for (let i = 1; i <= pageCount; i++) {
      console.log(`Wrote ${path.join(OUTPUT_DIR, `MR1135-Staged-Filled-Preview-page-${i}.png`)}`);
    }
    console.log(`Wrote ${path.join(OUTPUT_DIR, "staged-preview-binding-report.json")}`);
    console.log(`Wrote ${path.join(OUTPUT_DIR, "staged-preview-visual-qa-report.json")}`);
  } finally {
    await browser.close();
  }
}
