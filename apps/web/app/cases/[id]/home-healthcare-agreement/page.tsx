...
export default function HomeHealthcareAgreementPage() {
    ...
    useEffect(() => {
        ...
        void apiFetch<{ html_content: string }>(`/api/discharge/cases/${caseId}/home-healthcare-agreement/preview`, {
            method: "POST",
            body: JSON.stringify({ payload: {} }),
        })
            .then((res) => setPreviewHtml(res.html_content || ""))
            .catch((err: Error) => setMessage(err.message));
    }, [caseId, mobileLinked, requestedMethod, t]);
    ...
    const startFlow = async () => {
        ...
        try {
            ...
            if (res.verification_status === "notification_sent" && res.delivery_status === "sent") {
    ...
    return (
        <AuthGuard>
            <AppShell
                title={t("signaturePage.homeHealthcare.title")}
                subtitle={t("signaturePage.homeHealthcare.subtitle")}
                workflowCaseNav={{
                    caseId,
