...
const relatedConsents = [
  { id: "RC1", name: "Surgical Consent", nameAr: "موافقة جراحية", required: true },
  { id: "RC2", name: "General Anesthesia Consent", nameAr: "موافقة التخدير العام", required: true },
  { id: "RC3", name: "Blood Transfusion Consent", nameAr: "موافقة نقل الدم", required: false },
  { id: "RC4", name: "Telemedicine / Data Sharing Consent", nameAr: "موافقة الطب عن بُعد / مشاركة البيانات", required: false },
  { id: "RC5", name: "Photography / Teaching Consent", nameAr: "موافقة التصوير / التعليم", required: false },
];
...
const getComplexityLabel = (complexity: ProcedureCard["complexity"], lang: Props["lang"]) => {
  if (lang === "en") {
  ...
  const handleContinue = () => {
    const digitalConsentPayload = isCriticalCareSelected
      ? {
          ...
          amaRequired: refusalEnabled ? criticalCareConsentTemplate.refusalSection.amaRequired : false,
    ...
    <div className="p-8 space-y-6" dir={dir}>
      <div>
        <h2 className="text-[#002B5C]">
          {lang === "en" ? "Procedure & Consent Selection" : "اختيار الإجراء والموافقة"}
        </h2>
      ...
      <div className="bg-white border border-[#D8DCE3] rounded-lg overflow-hidden">
        <div className="px-5 py-3 border-b border-[#D8DCE3] bg-[#F4F6F9] flex items-center gap-2">
          ...
          <span className="text-sm font-semibold text-[#2F2F2F]">
            {lang === "en" ? "Recommended Procedures & Digital Templates" : "الإجراءات والقوالب الرقمية المقترحة"}
          </span>
                ...
                <div className="flex items-start gap-3 flex-wrap">
                  ...
                  {proc.isDigitalTemplate && (
                    <span className="text-xs bg-[#C9A13B]/10 border border-[#C9A13B]/30 text-[#8A6A12] rounded px-1.5 py-0.5 flex items-center gap-1 font-medium">
                      ...
                      {lang === "en" ? "IMC Digital Template" : "قالب رقمي معتمد للمركز"}
                    </span>
                  )}
                  ...
                  {proc.requiresAnesthesia && (
                    <span className="text-xs bg-amber-50 border border-amber-200 text-amber-700 rounded px-1.5 py-0.5 flex items-center gap-1">
                      ...
                      {lang === "en" ? "Anesthesia Required" : "يتطلب تخديرًا"}
                    </span>
                  )}
                </div>

                <div className="flex gap-4 mt-1.5 flex-wrap">
                  <span className="text-xs text-[#6B7280]">Code: {proc.code}</span>
        ...
        <div className="bg-white border border-[#D8DCE3] rounded-lg overflow-hidden">
          <div className="px-5 py-3 border-b border-[#D8DCE3] bg-[#F4F6F9]">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-semibold text-[#2F2F2F]">
                {lang === "en" ? "Critical Care Digital Consent Builder" : "بناء موافقة الرعاية الحرجة الرقمية"}
              </span>
            </div>
            <p className="text-xs text-[#6B7280] mt-1">
              {criticalCareConsentTemplate.formCode} · {criticalCareConsentTemplate.version} ·{" "}
              {lang === "en" ? "Structured bilingual template" : "قالب ثنائي اللغة مهيكل"}
            </p>
          </div>
          ...
          <div className="p-5 space-y-5">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              <label className="space-y-1.5">
                <span className="text-sm font-medium text-[#2F2F2F]">
                  {lang === "en" ? "ICU admission reason / clinical indication" : "سبب الدخول إلى وحدة العناية المركزة / الداعي الطبي"}
                </span>
                <textarea
                  value={icuAdmissionReason}
                  onChange={(event) => setIcuAdmissionReason(event.target.value)}
                  rows={4}
                  ...
                  placeholder={lang === "en" ? "Enter the clinical indication..." : "أدخل الداعي الطبي..."}
                />
              </label>

              <label className="space-y-1.5">
                <span className="text-sm font-medium text-[#2F2F2F]">
                  {lang === "en" ? "Physician additional notes" : "ملاحظات الطبيب الإضافية"}
                </span>
                <textarea
                  value={physicianAdditionalNotes}
                  onChange={(event) => setPhysicianAdditionalNotes(event.target.value)}
                  rows={4}
                  className="w-full rounded-md border border-[#D8DCE3] px-3 py-2 text-sm outline-none focus:border-[#002B5C] focus:ring-1 focus:ring-[#002B5C]"
            ...
            <div>
              <div className="flex items-center gap-2 mb-3">
                ...
                <h3 className="text-sm font-semibold text-[#2F2F2F]">
                  {lang === "en" ? "ICU Procedures Included in Consent" : "إجراءات العناية المركزة المشمولة في الموافقة"}
                </h3>
              </div>
                      ...
                      <div className="flex items-start gap-3">
                        <div
                          className={`w-4 h-4 mt-0.5 rounded border flex items-center justify-center shrink-0 ${
                            checked ? "bg-[#002B5C] border-[#002B5C]" : "border-[#D8DCE3] bg-white"
                          }`}
                        >
                        ...
                        <div>
                          <div className="text-sm font-medium text-[#2F2F2F]">
                            {lang === "en" ? procedure.title.en : procedure.title.ar}
                          </div>
                          <p className="text-xs text-[#6B7280] mt-1 leading-relaxed">
                            {lang === "en" ? procedure.uses.en : procedure.uses.ar}
                          </p>
                          <p className="text-xs text-red-700 mt-1 leading-relaxed">
                            {lang === "en" ? `Risks: ${procedure.risks.en}` : `المخاطر: ${procedure.risks.ar}`}
                          </p>
                        </div>
                      </div>
                    </button>
            ...
            <label className="space-y-1.5 block">
              <span className="text-sm font-medium text-[#2F2F2F] flex items-center gap-2">
                ...
                {lang === "en" ? "Additional procedure requested by physician" : "إجراء إضافي يطلبه الطبيب"}
              </span>
              <textarea
                value={additionalProcedure}
                onChange={(event) => setAdditionalProcedure(event.target.value)}
                rows={3}
                className="w-full rounded-md border border-[#D8DCE3] px-3 py-2 text-sm outline-none focus:border-[#002B5C] focus:ring-1 focus:ring-[#002B5C]"
                placeholder={lang === "en" ? "Optional additional procedure..." : "إجراء إضافي اختياري..."}
              />
            </label>
            ...
            <div className="rounded-lg border border-[#D8DCE3] bg-[#F9FAFB] p-4 space-y-3">
              <label className="flex items-center gap-3 cursor-pointer">
                ...
                />
                <span className="text-sm font-medium text-[#2F2F2F]">
                  {lang === "en" ? "Any procedure refused?" : "هل تم رفض أي إجراء؟"}
                </span>
              </label>
              ...
              {refusalEnabled && (
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                  <label className="space-y-1.5">
                    <span className="text-sm font-medium text-[#2F2F2F]">
                      {lang === "en" ? "Refused procedure" : "الإجراء المرفوض"}
                    </span>
                    <input
                      value={refusedProcedure}
                      onChange={(event) => setRefusedProcedure(event.target.value)}
                      className="w-full rounded-md border border-[#D8DCE3] px-3 py-2 text-sm outline-none focus:border-[#002B5C] focus:ring-1 focus:ring-[#002B5C]"
                    />
                  </label>
            ...
            <div className="rounded-lg border border-[#D8DCE3] bg-white p-4">
              <div className="flex items-center gap-2 mb-3">
                ...
                <h3 className="text-sm font-semibold text-[#2F2F2F]">
                  {lang === "en" ? "Bilingual Preview" : "معاينة ثنائية اللغة"}
                </h3>
              </div>

              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                <div className="rounded-md border border-[#EEF1F5] p-3">
                  <h4 className="text-xs font-semibold text-[#002B5C] mb-2">English</h4>
                ...
                <div className="rounded-md border border-[#EEF1F5] p-3" dir="rtl">
                  <h4 className="text-xs font-semibold text-[#002B5C] mb-2">العربية</h4>
                  <p className="text-xs text-[#2F2F2F] leading-relaxed">
                    {criticalCareConsentTemplate.introduction[0].ar}
                  </p>
                  <p className="text-xs text-[#6B7280] mt-2">
                    عدد إجراءات العناية المختارة: {selectedIcuProcedureDetails.length}
                  </p>
                </div>
              </div>
            </div>
        ...
        <div className="bg-white border border-[#D8DCE3] rounded-lg overflow-hidden">
          <div className="px-5 py-3 border-b border-[#D8DCE3] bg-[#F4F6F9]">
            <div className="flex items-center gap-2">
              <Info className="w-4 h-4 text-[#4B9CD3]" />
              <span className="text-sm font-semibold text-[#2F2F2F]">
                {lang === "en" ? "Consent Package Configuration" : "تهيئة حزمة الموافقة"}
              </span>
            </div>
            <p className="text-xs text-[#6B7280] mt-1">
              {lang === "en"
              ...
              <div
                ...
                onClick={() =>
                  setActiveConsents((previous) =>
                    previous.includes(rc.id)
                      ? previous.filter((id) => id !== rc.id)
                      : [...previous, rc.id]
                  )
                }
              >
