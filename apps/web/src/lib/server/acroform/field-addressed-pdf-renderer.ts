...
/**
 * Fail-closed guard: the canonical approved PDF must byte-for-byte match the
 * hash recorded in the verified manifest. Rendering onto any other PDF would
 * break deterministic field placement.
 */
function assertCanonicalPdfHashMatches(
  canonicalPdfBytes: Uint8Array,
  manifest: AcroFormTemplateManifest,
): void {
  const actualHash = sha256Hex(canonicalPdfBytes);
  const expectedHash = manifest.canonicalApprovedPdf.sha256;
  if (actualHash !== expectedHash) {
    throw new Error(
      `Canonical PDF hash mismatch for ${manifest.templateCode}. Expected ${expectedHash}, got ${actualHash}.`,
    );
  }
}

/**
 * Fail-closed guard: every value key must resolve to a manifest field. A stray
 * or renamed key is a binding regression and must not be silently ignored.
 */
function assertValueKeysAreKnown(
  values: Record<string, FieldAddressedRenderValue>,
  manifest: AcroFormTemplateManifest,
): void {
  const knownNames = new Set(manifest.fields.map((f) => f.name));
  const unknown = Object.keys(values).filter((key) => !knownNames.has(key));
  if (unknown.length > 0) {
    throw new Error(
      `Unknown field value keys cannot be rendered deterministically: ${unknown.join(", ")}`,
    );
  }
}

/**
 * Fail-closed guard: every manifest field marked required must have a value.
 * Optional fields may remain absent; required fields must not fall back to blank.
 */
export function assertRequiredFieldsPresent(
  ...
  const missing = manifest.fields
    .filter((field) => field.required && !provided.has(field.name))
    .map((field) => field.name);
  if (missing.length > 0) {
    throw new Error(
      `Required manifest fields are missing from values: ${missing.join(", ")}`,
    );
  }
}
