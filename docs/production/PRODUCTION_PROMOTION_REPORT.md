...
| **Pre‑OTP bootstrap hotfix (2026‑05‑27)** | `vercel deploy --prod` from commit `15e1b68` → `wathiqcare-discharge-refusal-o4o8sa504-wathiqcare.vercel.app` (Ready in 2m, Aliased to `wathiqcare.online`). **Supersedes** `i866h3wek`. Resolves deterministic 401 "Missing public signing session" on true cold opens from Mail/SMS. `GET /api/public-signing/document/[token]` now returns a discriminated `{ phase: "pre-otp", bootstrap: { facilityName, templateTitleAr, templateTitleEn, locale, educationRequired, … } }` payload when no session cookie is present; post‑OTP responses unchanged (full `PublicSigningDocumentPayload`). No PHI exposed pre‑OTP. Cookie attrs (HttpOnly/Secure/lax/.wathiqcare.online/Max-Age=1797) preserved. Cold‑open E2E + Phase17 10/10 PASS. See [PRE_OTP_BOOTSTRAP_FLOW.md](../../PRE_OTP_BOOTSTRAP_FLOW.md). |
