...
test("module access applies role isolation and platform override", () => {
    ...
    assert.equal(canAccessModule("wathiqnote", { role: "doctor", platformRole: null }), false);
    assert.equal(canAccessModule("wathiqnote", { role: "finance_officer", platformRole: null }), true);
    ...
    assert.equal(
        canAccessModule("wathiqnote", { role: "tenant_user", platformRole: "platform_admin" }),
        true,
    );
    assert.equal(
