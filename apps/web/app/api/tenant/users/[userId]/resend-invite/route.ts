...
export async function POST(request: NextRequest, context: RouteContext) {
  try {
    ...
    const inviteLink = `${appUrl}/auth/accept-invite?token=${encodeURIComponent(
      invitation.token,
    )}`;
    ...
    const emailHtml = buildWathiqCareEmailHtml({
      ...
      bodyHtml: `<p style="margin:0 0 12px;font-size:15px;color:#334155;line-height:1.7;">Hi ${user.fullName},</p>
