...
export function ConsentPreviewModal({
  ...
  const displayPdfUrl = draftPdfUrl || approvedPdfUrl;
    ...
    <Dialog open={open} onOpenChange={(isOpen) => !isOpen && onClose()}>
      <DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto">
        ...
        <div className="space-y-4 text-sm">
          ...
          {hasApprovedPdfSource && displayPdfUrl ? (
            <div className="space-y-3">
              ...
              {draftPdfUrl ? (
                <div className="rounded-lg border border-emerald-200 bg-emerald-50 px-3 py-2 text-[11px] font-medium text-emerald-800">
                  {isAr
                    ? "يتم عرض مسودة الـ PDF المكتملة من الطبيب. النموذج المعتمد الأصلي يبقى ثابتاً."
                    : "Showing the physician-completed draft PDF. The original approved form remains immutable."}
                </div>
              ) : null}
              <div className="text-[11px] text-slate-500">
                <a href={displayPdfUrl} target="_blank" rel="noreferrer" className="text-blue-600 underline underline-offset-2">
                  {isAr ? "فتح ملف الموافقة في نافذة جديدة" : "Open the consent PDF in a new tab"}
                </a>
              </div>
            </div>
          ) : null}
              ...
              <ul className="list-disc list-inside text-[11px] text-slate-600 space-y-1">
                {assembly.riskDisclosures.map((risk) => (
                  <li key={risk.id}>
                    ...
                    {risk.incidenceRate && <span className="text-slate-400 ml-1">({risk.incidenceRate})</span>}
                  </li>
                ))}
              </ul>
            </div>
              ...
              <ul className="list-disc list-inside text-[11px] text-slate-600 space-y-1">
                {assembly.educationMaterials.map((material) => (
                  <li key={material.id}>
                    ...
                    {material.status !== "PUBLISHED" && <span className="ml-1 text-amber-600 font-medium">[Draft]</span>}
                  </li>
                ))}
              </ul>
            </div>
            ...
            <div className="space-y-2 border-t border-dashed border-slate-200 pt-3">
              ...
              <div className="grid grid-cols-2 gap-3 opacity-70">
                {draftIllustrations.map((illustration) => (
                  <div key={illustration.id} className="space-y-1">
                    <p className="text-[10px] font-medium text-slate-700 truncate">{illustration.procedureNameEn}</p>
