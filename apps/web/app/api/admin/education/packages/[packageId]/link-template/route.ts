...
export async function POST(request: NextRequest, context: RouteContext) {
  try {
    const [{ requireAuth, requireTenantId }, { isAdministrator }, { linkEducationPackageToConsentTemplate }] = await Promise.all([
    ...
    const result = await linkEducationPackageToConsentTemplate({
      tenantId: requireTenantId(auth),
      packageId,
      actorUserId: auth.sub,
      consentTemplateId: body?.consentTemplateId || "",
      consentTemplateVersionId: body?.consentTemplateVersionId || "",
